...
//! against the router constructed by `go_clean::build_router`.
...
use go_clean::config::env::Env;
use go_clean::utils::jwt::generate_token_pair;
use go_clean::{AppState, build_router};
...
async fn spawn_state() -> (AppState, ContainerAsync<Postgres>) {
    ...
    let database_url = format!("postgres://postgres:postgres@127.0.0.1:{host_port}/go_clean_test");
