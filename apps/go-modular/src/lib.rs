...
/// `axum::extract::State`.
///
/// At D-INFRA-11 this only holds `config` and `pool`. Services
/// (`Arc<UserService>`, `Arc<AuthService>`, `Arc<Mailer>`) land as
/// their respective modules are implemented in D-USER / D-AUTH /
/// D-SMTP phases.
#[derive(Clone)]
pub struct AppState {
